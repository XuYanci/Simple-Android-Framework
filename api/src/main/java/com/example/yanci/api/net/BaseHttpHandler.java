...
import com.example.yanci.api.util.Encrypt.Des;
...
public class BaseHttpHandler {
    ...
    // HTTP 请求 URL
    public static String REQUEST_LOGINBYPHONE = "loginbyphone";


    // 服务器URL
    private final static  String SERVER_URL = "http://app.test3.max99.cn";
    private final static  String SERVER_LOGIN_SUFFIX = "/index.php?m=Home&c=User&a=phoneLogin";

    // 编码类型
    private final static String ENCODE_TYPE = "UTF-8";
    private final static String AID = "2";
    ...
    // 编解码器
    private  Des desInstance;
    private final static String DES_KEY = "maxer123";

    private  BaseHttpHandler() {
        desInstance = new Des(DES_KEY);
    }
    ...
    private String RequestUrl(String Request)  {
        ...
        if (Request == REQUEST_LOGINBYPHONE) {
            return String.format(SERVER_URL + SERVER_LOGIN_SUFFIX + "&aid=%1$s&t=%2$s",
                    URLEncoder.encode(desInstance.encryptWithBase64(AID)),
                    URLEncoder.encode(desInstance.encryptWithBase64(System.currentTimeMillis() / 1000 + "")));
        }
    ...
    public  <T> T HttpPostRequest(String request,Map<String, String> paramsMap, Type typeOfT) throws IOException{
        ...
        if (connection.getResponseCode() == 200) {
            ...
            connection.disconnect();
            final  String result = new String(baos.toByteArray());
            final  String desResult = desInstance.descryptBase64(result);
            Gson gson = new Gson();
            return gson.fromJson(desResult, typeOfT);
        }
    ...
    private  String joinParams(Map<String,String> paramsMap) {
        ...
        for (String key : paramsMap.keySet()) {
            ...
            try {
                stringBuilder.append(URLEncoder.encode(desInstance.encryptWithBase64(paramsMap.get(key)), ENCODE_TYPE));
